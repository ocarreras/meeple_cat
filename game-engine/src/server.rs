...
fn resolve_eval_fn(
    ...
    Box<dyn Fn(&serde_json::Value, &models::Phase, &str, &[models::Player], &dyn GamePlugin) -> f64 + Send + Sync>,
> {
    if game_id != "carcassonne" {
        return None;
    }
    match eval_profile {
...
impl GameEngineService for GameEngineServer {
    ...
    async fn mcts_search(
        ...
        Ok(Response::new(MctsSearchResponse {
            ...
            iterations_run: params.num_simulations as i32,
            elapsed_ms,
        }))
    }
