...
import { getPlacedKites, NUM_ORIENTATIONS, type Kite } from '@/lib/einsteinPieces';
...
const HEX_SIZE = 40;
const PLAYER_COLORS = ['#3b82f6', '#f97316']; // blue, orange — matching physical game
const PLAYER_FILL_COLORS = ['rgba(59, 130, 246, 0.6)', 'rgba(249, 115, 22, 0.6)'];
const GHOST_VALID_COLOR = 'rgba(34, 197, 94, 0.5)';
const GHOST_INVALID_COLOR = 'rgba(156, 163, 175, 0.3)';
...
export default function EinsteinDojoBoard({
  gameData,
  players,
  onHexClicked,
  isMyTurn,
  ...
  // Build player color map: player_id → seat_index
  ...
  useEffect(() => {
    ...
    const screenToHex = (clientX: number, clientY: number) => {
      ...
      const worldX = (canvasX - canvas.width / 2 - cam.x) / cam.zoom;
      const worldY = (canvasY - canvas.height / 2 - cam.y) / cam.zoom;

      // Reverse hexToPixel: x = 3/2 * size * q, y = sqrt3 * size * (r + q/2)
    ...
    const handleTouchEnd = (e: TouchEvent) => {
      ...
      if (!touchMovedRef.current && touchStartRef.current && e.changedTouches.length === 1) {
        ...
        if (isMyTurnRef.current && phaseRef.current === 'place_tile') {
          const hex = screenToHex(touch.clientX, touch.clientY);
  ...
  const screenToWorld = useCallback((screenX: number, screenY: number) => {
    ...
    const rect = canvas.getBoundingClientRect();
    return {
      x: (screenX - rect.left - canvas.width / 2 - camera.x) / camera.zoom,
      y: (screenY - rect.top - canvas.height / 2 - camera.y) / camera.zoom,
    };
  }, [camera]);
  ...
  const handleMouseUp = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    if (isDragging) {
      ...
      if (dragDistance < 5 && isMyTurn && phase === 'place_tile') {
        ...
        // Convert world to hex
        const q = (2 / 3) * world.x / HEX_SIZE;
        const r = (-1 / 3) * world.x / HEX_SIZE + (Math.sqrt(3) / 3) * world.y / HEX_SIZE;
        const s = -q - r;
        let rq = Math.round(q);
        let rr = Math.round(r);
        const rs = Math.round(s);
    ...
    setIsDragging(false);
  }, [isDragging, dragStart, camera, isMyTurn, phase, screenToWorld, onHexClicked]);
  ...
  useEffect(() => {
    ...
    // Draw subtle hex grid around occupied area
    ...
    if (occupiedHexes.size === 0) {
      // Empty board — show small grid around origin
      for (let q = -3; q <= 3; q++) {
        for (let r = -3; r <= 3; r++) {
          gridHexes.add(`${q},${r}`);
        }
      ...
      // Show hexes within 2 steps of occupied
    ...
    for (const piece of gameData.board.placed_pieces) {
      ...
      for (const [q, r, k] of kites) {
        const { x: cx, y: cy } = hexToPixel(q, r, HEX_SIZE);
        const poly = kitePolygon(cx, cy, HEX_SIZE, k);
    ...
    if (ghostPiece) {
      ...
      for (const [q, r, k] of kites) {
        ...
        ctx.fillStyle = fillColor;
        ctx.fill();
        ctx.strokeStyle = ghostPiece.valid ? '#22c55e' : '#9ca3af';
        ctx.lineWidth = 1.5;
        ctx.stroke();
      }
